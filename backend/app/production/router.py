...
from .schemas import PTCreate, PTRead, PTUpdate, PTListResponse, CommentCreate, CommentRead, AssignmentCreate, StatusChangeRequest, TagCreate, DependencyCreate, WatcherCreate, TimelineEntry, AttachmentRead, AssignmentRead, HistoryEntry, AttachmentCreate
...
async def create_pt(payload: PTCreate, session: AsyncSession = Depends(get_session)):
...
async def list_pts(limit: int = 25, offset: int = 0, search: Optional[str] = None, status: Optional[str] = None, category: Optional[str] = None, priority: Optional[str] = None, tag: Optional[str] = None, assignee_id: Optional[str] = None, sort_by: str = 'created_at', order: str = 'desc', session: AsyncSession = Depends(get_session)):
    svc = ProductionTicketService(session)
...
async def get_pt(ticket_id: str, session: AsyncSession = Depends(get_session)):
    svc = ProductionTicketService(session)
...
async def update_pt(ticket_id: str, payload: PTUpdate, session: AsyncSession = Depends(get_session)):
    svc = ProductionTicketService(session)
    try:
        updated = await svc.update_ticket(ticket_id, payload)
...
async def delete_pt(ticket_id: str, session: AsyncSession = Depends(get_session)):
    svc = ProductionTicketService(session)
    await svc.repo.delete(ticket_id)
    return None

@router.post('/{ticket_id}/status', response_model=PTRead)
async def change_status(ticket_id: str, payload: StatusChangeRequest, session: AsyncSession = Depends(get_session)):
    svc = ProductionTicketService(session)
    try:
        pt = await svc.change_status(ticket_id, payload)
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    if not pt:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='PT not found')
...
async def add_comment(ticket_id: str, payload: CommentCreate, session: AsyncSession = Depends(get_session)):
    svc = ProductionTicketService(session)
    comment = await svc.add_comment(ticket_id, payload)
    return comment
...
async def assign(ticket_id: str, payload: AssignmentCreate, session: AsyncSession = Depends(get_session)):
    svc = ProductionTicketService(session)
    assigns = await svc.assign(ticket_id, payload)
    return assigns

@router.get('/{ticket_id}/assignments', response_model=list[AssignmentRead])
async def list_assignments(ticket_id: str, session: AsyncSession = Depends(get_session)):
    svc = ProductionTicketService(session)
...
async def add_attachment(ticket_id: str, payload: AttachmentCreate, session: AsyncSession = Depends(get_session)):
    svc = ProductionTicketService(session)
    att = await svc.add_attachment(ticket_id, payload.filename, payload.url, uploader_id=None)
    return att

@router.get('/{ticket_id}/attachments', response_model=list[AttachmentRead])
async def list_attachments(ticket_id: str, session: AsyncSession = Depends(get_session)):
    svc = ProductionTicketService(session)
    items = await svc.list_attachments(ticket_id)
    return items

@router.post('/{ticket_id}/tags', status_code=201)
async def add_tag(ticket_id: str, payload: TagCreate, session: AsyncSession = Depends(get_session)):
    svc = ProductionTicketService(session)
    t = await svc.add_tag(ticket_id, payload.name)
    return {"id": str(t.id), "name": t.name}

@router.delete('/{ticket_id}/tags')
async def remove_tag(ticket_id: str, name: str, session: AsyncSession = Depends(get_session)):
    svc = ProductionTicketService(session)
    await svc.remove_tag(ticket_id, name)
    return {"ok": True}

@router.post('/{ticket_id}/watchers', status_code=201)
async def add_watcher(ticket_id: str, payload: WatcherCreate, session: AsyncSession = Depends(get_session)):
    svc = ProductionTicketService(session)
    w = await svc.add_watcher(ticket_id, payload.user_id)
    return {"id": str(w.id)}

@router.delete('/{ticket_id}/watchers')
async def remove_watcher(ticket_id: str, user_id: str, session: AsyncSession = Depends(get_session)):
    svc = ProductionTicketService(session)
    await svc.remove_watcher(ticket_id, user_id)
    return {"ok": True}

@router.post('/{ticket_id}/dependencies', status_code=201)
async def add_dependency(ticket_id: str, payload: DependencyCreate, session: AsyncSession = Depends(get_session)):
    svc = ProductionTicketService(session)
...
async def get_timeline(ticket_id: str, session: AsyncSession = Depends(get_session)):
    svc = ProductionTicketService(session)
    items = await svc.list_timeline(ticket_id)
    return items

@router.get('/{ticket_id}/history', response_model=list[HistoryEntry])
async def get_history(ticket_id: str, session: AsyncSession = Depends(get_session)):
    svc = ProductionTicketService(session)
